  ...
  return (
    <div className="px-4 pt-12 pb-4 space-y-8 max-w-lg mx-auto">
      ...
      <div>
        ...
        <div className="flex gap-3 mt-4">
          <button
            ...
            className="px-5 py-2.5 rounded-full bg-primary text-primary-foreground text-sm font-body font-medium hover:opacity-90 transition-opacity"
          >
            Join your neighborhood
          </button>
          <button
            onClick={() => router.push('/browse')}
            className="px-5 py-2.5 rounded-full border border-primary text-primary text-sm font-body font-medium hover:bg-primary/5 transition-colors"
          >
            Browse swaps
          </button>
        </div>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground font-body mb-4">Helping moms share more, waste less, and build real community.</p>
        <div className="grid grid-cols-3 gap-3 text-center">
          {pillars.map((pillar) => (
            <div key={pillar.title} className="flex flex-col items-center gap-1.5">
              ...
              <p className="text-[10px] leading-tight text-muted-foreground font-body">{pillar.desc}</p>
            </div>
          ))}
        </div>
      </motion.div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
          {Object.entries(categoryLabels).map(([key, label]) => (
            <button
              ...
              className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full bg-card border border-border text-sm font-body text-foreground hover:border-primary/40 transition-colors"
            >
      ...
      >
        ...
        <p className="text-2xl font-heading text-secondary mt-1">1,247 items passed from one family to another</p>
